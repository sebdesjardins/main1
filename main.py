...
# Dictionnaire global : {nom_arduino: {"ws": websocket, "last_seen": datetime}}
connected_arduinos = {}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        print(f"⚠️ Arduino déconnecté : {arduino_name} ({e})")
    finally:
...
async def get_connected():
    """Retourne la liste des Arduinos connectés (sans les objets WebSocket)"""
    try:
        now = datetime.now()
        result = {}
        ...
        print(f"❌ Erreur dans /get_arduino_connected : {e}")
...
async def reboot_arduino(arduino: str):
    ...
    if arduino not in connected_arduinos:
        return JSONResponse(
            {"status": "error", "message": "Arduino non connecté"},
            status_code=404,
        )
    ...
    try:
        ...
        return JSONResponse(
            {"status": "ok", "message": f"Commande REBOOT envoyée à {arduino}"}
        )
    except Exception as e:
        print(f"❌ Erreur d’envoi REBOOT : {e}")
